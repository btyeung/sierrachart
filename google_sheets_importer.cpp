...
#include <sstream>
#include <vector>
SCDLLName("Google Sheets Levels Importer v2.02")
...
SCSFExport scsf_GoogleSheetsLevelsImporter(SCStudyInterfaceRef sc)
{
    ...
    // GSheets tab specific name
    SCInputRef i_TabName = sc.Input[11];
    ...
    if (sc.SetDefaults)
    {
        ...
        // Example with tab specification https://docs.google.com/spreadsheets/d/1yNSdWSh12_XrGGhpMfjlVN8m1YcQpQf0M2sKY30n3xw/gviz/tq?tqx=out:csv&sheet=SC
        i_FilePath.SetString("https://docs.google.com/spreadsheets/d/1yNSdWSh12_XrGGhpMfjlVN8m1YcQpQf0M2sKY30n3xw");
        ...
        i_RecalcInterval.SetInt(1);

        i_TabName.Name = "Google Sheets Tab Name";
        i_TabName.SetString("SC-NQ");
    ...
    SCString TabName = i_TabName.GetString();

    SCString Url = i_FilePath.GetString();
    // add specific code to output as CSV attachment from Google Sheets

    sc.AddMessageToLog(Url, 1);
    Url.Format("%s/gviz/tq?tqx=out:csv", Url.GetChars());
    sc.AddMessageToLog(Url.GetChars(), 1);

    // TODO: add tab name if exists
    if (TabName != "")
    {
        sc.AddMessageToLog("Tab Name Specified, adding to URL", 1);

        // Format of google sheet with tab name as /gviz/tq?tqx=out:csv;outFileName:data&sheet=”
        Url.Format("%s&sheet=%s", Url.GetChars(), TabName.GetChars());
    }
    else
    {
        sc.AddMessageToLog("Tab Name NOT Specified, defaulting to normal URL", 1);
    }
    ...
    // sc.AddMessageToLog("Google sheet sync running... Index: " + char(sc.Index) + " recalc: " + char(RecalcIntervalMin), 1);
    sc.AddMessageToLog(GSheetMessage, 1);
    ...
    if (sc.Index == 0 || RecalcIntervalMin > 0)
    {
        if (RequestState == REQUEST_NOT_SENT)
        {
            ...
            sc.AddMessageToLog("HTTP Request sent.", 1);
        }
    }
    ...
    if (RequestState == REQUEST_SENT && sc.HTTPResponse != "")
    {
        ...
        sc.AddMessageToLog("Attempting to contact GSheets", 1);
    ...
    else if (RequestState == REQUEST_SENT && sc.HTTPResponse == "")
    {
        ...
        sc.AddMessageToLog("Still waiting on HTTP Request...", 1);
        return;
    }
    ...
    for (std::string line; getline(input, line);)
    {
        ...
        for (char *i : tokens)
        {
            ...
            else if (idx == 7)
            {
                ...
                if (textalignment > 0)
                    Tool.TextAlignment = textalignment;
            }
